...
class Paint(object):
    ...
    def __init__(self):
        ...
        self.reset_button = Button(self.root, text='Clear all', command=self.clear_all)
        self.reset_button.grid(row=0, column=3)

        self.choose_size_button = Scale(self.root, from_=1, to=10, orient=HORIZONTAL)
        self.choose_size_button.grid(row=1, column=0)

        self.choose_eraser_size_button = Scale(self.root, from_=1, to=10, orient=HORIZONTAL)
        self.choose_eraser_size_button.grid(row=1, column=1)

        self.save_button = Button(self.root, text='Save', command=self.save)
        self.save_button.grid(row=0, column=4)

        self.c = Canvas(self.root, bg=self.DEFAULT_BG, width=self.DEFAULT_WIDTH, height=self.DEFAULT_HEIGHT)
